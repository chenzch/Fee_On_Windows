...
#define MEMACC_SEMA4_SW_MAJOR_VERSION_C               6
...
Std_ReturnType MemAcc_Sema4_RequestLock(uint8 MemHwResource)
{
    ...
    if (MEMACC_SEMA4_INVALID_GATE != Sema4Gate)
    {
...
void MemAcc_Sema4_ReleaseLock(uint8 MemHwResource)
{
    ...
    /* Get the Sema4 Gate for the hardware resource*/
    Sema4Gate = (uint8)Memacc_u8Sema4Gate[MemHwResource];

    /* Get the status of the sema4 gate*/
    semaphoreStatus = Rm_SemaphoreGetStatus(Sema4Gate);
...
Std_ReturnType MemAcc_Sema4_IsLocked(uint8 MemHwResource)
{
    ...
    /* If the Sema4 is held by this core */
    if (domainID == semaphoreStatus)
    {
